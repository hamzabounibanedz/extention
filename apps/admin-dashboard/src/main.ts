...
function render(): void {
  ...
  const listSearchInput = el('input', {
    type: 'search',
    id: 'codes-search',
    placeholder: t(lang, 'list_search_placeholder'),
    'aria-label': t(lang, 'list_search_label'),
  }) as HTMLInputElement;
  const listSearchLabel = el('label', {
    for: 'codes-search',
    className: 'sr-only',
    textContent: t(lang, 'list_search_label'),
  });

  const toolbar = el('div', { className: 'toolbar' });
  toolbar.append(listSearchLabel, listSearchInput, refreshBtn);
  ...
  async function loadList(): Promise<void> {
    ...
    try {
      const search = listSearchInput.value.trim();
      const qs = search ? `&search=${encodeURIComponent(search)}` : '';
      const data = await api<{ items: LicenseItem[] }>(`/admin/v1/license-codes?limit=50${qs}`);
